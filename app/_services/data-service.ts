...
import { IBooking, ICabin, IError, IGuests } from "./schemas";
...
export async function getCabinPrice(id: string) {
  const { data, error }: { data: ICabin | null; error: any } = await supabase
    .from("cabins")
...
export async function getBookings(guestId: string) {
  const { data, error, count } = await supabase
    ...
    .select(
      "id, created_at, startDate, endDate, numNights, numGuests, totalPrice, guestId, cabinId, cabins(*)"
    )
    .eq("guestId", guestId)
    .order("startDate");
...
export async function createGuest(newGuest: IGuests) {
